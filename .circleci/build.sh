...
git clone --depth=1 https://gitlab.com/Forenche/https-github.com-stormbreaker-project-kernel_xiaomi_surya -b  master  kernel
cd kernel
...
function push() {
    ...
    curl -F document=@$ZIP "https://api.telegram.org/bot$token/sendDocument" \
        ...
        -F "parse_mode=html" \
        -F caption="Build took $(($DIFF / 60)) minute(s) and $(($DIFF % 60)) second(s). | For <b>Poco x3 (surya) for AOSP-Alpha</b> | <b>Proton clang 12</b>"
}
# Fin Error
function finerr() {
    curl -s -X POST "https://api.telegram.org/bot$token/sendMessage" \
